    ...
    public:
        ...
        * \return the final step t computed by the back-tracking line search */
        ...
        static double backtracking_line_search_newton_with_infeasible_start(Problem *_problem,
            ...
            // https://slides.cgg.unibe.ch/aopt22/09-EqualityConstrainedOptimization-I-deck.html#/19/0/3
            do {
                if (iter > 0)
                    t *= _beta;
                x = _x + t * _dx;
                ...
                const double rdual = (g + _A.transpose() * nu).squaredNorm();
                const double rpri = (_A * x - _b).squaredNorm();
                norm_res = std::sqrt(rdual + rpri);
                iter++;
            } while(norm_res > (1 - _alpha * t) * _initial_res || t < 1e-7);
