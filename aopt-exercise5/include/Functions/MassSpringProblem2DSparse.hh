    ...
    public:
        ...
        virtual double eval_f(const Vec &_x) override {
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); i++) {
                ...
                coeffw[0] = weights_[i];
                coeffw[1] = desired_points_[2 * i];
                coeffw[2] = desired_points_[2 * i + 1];

                energy += cse_.eval_f(cs_xe_, coeffw);
            }
        ...
        virtual void eval_gradient(const Vec &_x, Vec &_g) override {
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); i++) {
                ...
                cse_.eval_gradient(cs_xe_, coeffw, cs_ge_);
        ...
        virtual void eval_hessian(const Vec &_x, SMat& _h) override {
           ...
           //------------------------------------------------------//
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); i++) {
                ...
                cse_.eval_hessian(cs_xe_, coeffw, cs_he_);

                //copy to global
                triplets.emplace_back(2*attached_node_indices_[i], 2*attached_node_indices_[i], cs_he_(0,0));
                triplets.emplace_back(2*attached_node_indices_[i], 2*attached_node_indices_[i]+1, cs_he_(0,1));
                triplets.emplace_back(2*attached_node_indices_[i]+1, 2*attached_node_indices_[i], cs_he_(1,0));
                triplets.emplace_back(2*attached_node_indices_[i]+1, 2*attached_node_indices_[i]+1, cs_he_(1,1));
            }
            //------------------------------------------------------//
