...
//== CLASS DEFINITION =========================================================
    ...
    public:
        ...
        inline virtual double eval_f(const Vec &_x, const Vec &_coeffs) override {
            ...
            return sqrt(_coeffs[0]) * ((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3])*(_x[1]-_x[3]) - _coeffs[1]*_coeffs[1]);
            //------------------------------------------------------//
        }
        ...
        inline virtual void eval_gradient(const Vec &_x, const Vec &_coeffs, Vec &_g) override {
            ...
            _g << 2*sqrt(_coeffs[0]) * (_x[0] - _x[2]),
                  2*sqrt(_coeffs[0]) * (_x[1] - _x[3]),
                  -2*sqrt(_coeffs[0]) * (_x[0] - _x[2]),
                  -2*sqrt(_coeffs[0]) * (_x[1] - _x[3]);
