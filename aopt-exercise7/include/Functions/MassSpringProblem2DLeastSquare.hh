    ...
    private:
        ...
        void eval_r(const Vec &_x, Vec& _r) {
            ...
            for (size_t i = 0; i < attached_node_indices_.size(); i++) {
                coeff1 << weights_[i], desired_points_[2 * i];
        ...
        void eval_jacobian(const Vec &_x, SMat &_J) {
            ...
            for (size_t i = 0; i < springs_.size(); i++) {
                ...
                if (spring_type_ == WITHOUT_LENGTH) {
                    ...
                    xe_ << _x[2 * springs_[i].first],  _x[2 * springs_[i].first + 1],
                             _x[2 * springs_[i].second], _x[2 * springs_[i].second + 1];
                    func_.eval_gradient(xe_, coeff, ge_);
            ...
            Vec coeff1(2);
            for (size_t i = 0; i < attached_node_indices_.size(); i++) {
                const auto offset = num_rj + 2 * i;

                coeff1 << weights_[i], desired_points_[2 * i];
                cs_xe_[0] = _x[2 * attached_node_indices_[i]];
                cse_.eval_gradient(cs_xe_, coeff1, cs_ge_);
                triplets.emplace_back(offset, 2 * attached_node_indices_[i], cs_ge_[0]); // see comment on top for 0 index

                // grad for ending node point
                coeff1[1] = desired_points_[2 * i + 1];
                cs_xe_[0] = _x[2 * attached_node_indices_[i] + 1];
                cse_.eval_gradient(cs_xe_, coeff1, cs_ge_);
                triplets.emplace_back(offset + 1, 2 * attached_node_indices_[i] + 1, cs_ge_[0]); // see comment on top for 0 index
            }
